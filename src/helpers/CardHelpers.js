 ...
 * @returns
 */
export const AddCardToPlayerBoardCards = ({ G, ctx, myPlayerID, ...rest }, card) => {
 ...
 */
export const AddAnyCardToPlayerActions = ({ G, ctx, myPlayerID, ...rest }, pickedCard) => {
    var _a, _b;
    let finalPickedCard = pickedCard;
    const player = G.publicPlayers[Number(myPlayerID)];
    if (player === undefined) {
        return ThrowMyError({ G, ctx, ...rest }, ErrorNames.PublicPlayerWithCurrentIdIsUndefined, myPlayerID);
    }
    let startGiant = false, _exhaustiveCheck;
        ...
        case CardTypeRusNames.DwarfCard:
                    ...
                    switch (pickedCard.playerSuit) {
                        case SuitNames.blacksmith:
                            if (CheckPlayerHasBuff({ G, ctx, myPlayerID, ...rest }, GiantBuffNames.PlayerHasActiveGiantThrivaldi)) {
                                AddActionsToStack({ G, ctx, myPlayerID, ...rest }, [AllStackData.activateGiantAbilityOrPickCard(GiantNames.Thrivaldi, pickedCard)]);
                                startGiant = true;
                            }
                            break;
                        case SuitNames.explorer:
                            if (CheckPlayerHasBuff({ G, ctx, myPlayerID, ...rest }, GiantBuffNames.PlayerHasActiveGiantGymir)) {
                                AddActionsToStack({ G, ctx, myPlayerID, ...rest }, [AllStackData.activateGiantAbilityOrPickCard(GiantNames.Gymir, pickedCard)]);
                                startGiant = true;
                            }
                            break;
                        case SuitNames.hunter:
                            if (CheckPlayerHasBuff({ G, ctx, myPlayerID, ...rest }, GiantBuffNames.PlayerHasActiveGiantSkymir)) {
                                AddActionsToStack({ G, ctx, myPlayerID, ...rest }, [AllStackData.activateGiantAbilityOrPickCard(GiantNames.Skymir, pickedCard)]);
                                startGiant = true;
                            }
                            break;
                        case SuitNames.miner:
                            if (CheckPlayerHasBuff({ G, ctx, myPlayerID, ...rest }, GiantBuffNames.PlayerHasActiveGiantHrungnir)) {
                                AddActionsToStack({ G, ctx, myPlayerID, ...rest }, [AllStackData.activateGiantAbilityOrPickCard(GiantNames.Hrungnir, pickedCard)]);
                                startGiant = true;
                            }
                            break;
                        case SuitNames.warrior:
                            if (CheckPlayerHasBuff({ G, ctx, myPlayerID, ...rest }, GiantBuffNames.PlayerHasActiveGiantSurt)) {
                                AddActionsToStack({ G, ctx, myPlayerID, ...rest }, [AllStackData.activateGiantAbilityOrPickCard(GiantNames.Surt, pickedCard)]);
                                startGiant = true;
                            }
                            break;
                        default:
            ...
            if (!startGiant) {
                if (G.expansions.Idavoll.active
                    ...
                    AddActionsToStack({ G, ctx, myPlayerID, ...rest }, [AllStackData.activateGodAbilityOrNot(GodNames.Frigg, pickedCard)]);
                    return;
                }
                else {
    ...
    if (`stack` in pickedCard && pickedCard.stack !== undefined) {
        if (G.mode === GameModeNames.Solo && myPlayerID === `1`) {
            ...
            if (!(G.expansions.Idavoll.active && (pickedCard.name === HeroNames.Bonfur
                ...
                || (pickedCard.name === HeroNames.Dagda && CheckPlayerHasBuff({ G, ctx, myPlayerID, ...rest }, MythicalAnimalBuffNames.DagdaDiscardOnlyOneCards)))
                ...
                AddActionsToStack({ G, ctx, myPlayerID, ...rest }, pickedCard.stack.player, pickedCard);
            }
        }
    }
