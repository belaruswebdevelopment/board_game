    ...
    distinction: {
        ...
        awarding: (G, ctx, player) => {
            if (G.tierToEnd !== 0) {
                ...
                AddDataToLog(G, "game", `Игрок ${player.nickname} получил по знаку отличия кузнецов карту 
                Главного кузнеца.`);
                ctx.events.endTurn();
            }
        },
    ...
    distinction: {
        ...
        awarding: (G, ctx, player) => {
            if (G.tierToEnd !== 0) {
                ...
                AddDataToLog(G, "game", `Игрок ${player.nickname} обменял по знаку отличия охотников свою 
                монету с номиналом 0 на особую монету с номиналом 3.`);
                ctx.events.endTurn();
            }
        },
    },
    ...
    distinction: {
        ...
        awarding: (G, ctx, player) => {
            if (G.tierToEnd !== 0) {
                ...
                AddDataToLog(G, "game", `Игрок ${player.nickname} обменял по знаку отличия горняков свой 
                кристалл на особый кристалл 6.`);
                ctx.events.endTurn();
            } else {
