...
import type { AllCoinsValueType, AllInitialCoins, AllInitialTradingCoinConfig, AllPriorityValueType, AllRoyalCoinConfig, BasicVidofnirVedrfolnirUpgradeValueType, CampCardArray, CampCardType, CanBeNullType, CoinType, DwarfDeckCardType, ExplorerDistinctionCards, HeroCard, HeroesForSoloGameArrayType, HeroesForSoloGameForStrategyBotAndvariArray, HeroesInitialForSoloGameForBotAndvariArray, IndexOf, InitialCoinType, MercenariesConfigType, MythologicalCreatureCardType, MythologicalCreatureCardsForGiantSkymirArray, OneOrTwoOrThreeOrFour, PlayerCoinIdType, PlayerPouchCoinIdType, PlayerStack, PlayerTavernCoinIdType, PrivatePlayerBoardCoins, PrivatePlayerHandCoins, PublicPlayerBoardCoins, PublicPlayerCoinType, PublicPlayerHandCoins, RoyalOfferingsConfig, SecretAllCampDecks, SecretAllDwarfDecks, Stack, TavernCardIdType, TavernsConfigType, TavernsType, TierType, TradingCoinsType, TradingCoinsValueType, UpgradableCoinType, UpgradableCoinValueType, ZeroOrOneOrTwoOrThreeOrFourType, ZeroOrOneOrTwoOrThreeType, ZeroOrOneOrTwoType } from "../typescript/interfaces";
...
export function AssertTavernCardId(number: number): asserts number is TavernCardIdType {
...
export function AssertZeroOrOneOrTwo(number: number): asserts number is ZeroOrOneOrTwoType {
    if (!(number >= 0 || number <= 2)) {
        throw new Error(`No value '${number}' of ZeroOrOneOrTwoType.`);
    }
}

export function AssertZeroOrOneOrTwoOrThree(number: number): asserts number is ZeroOrOneOrTwoOrThreeType {
    if (!(number >= 0 || number <= 3)) {
        throw new Error(`No value '${number}' of ZeroOrOneOrTwoOrThreeType.`);
    }
}

export function AssertOneOrTwoOrThreeOrFour(number: number): asserts number is OneOrTwoOrThreeOrFour {
    if (!(number >= 1 || number <= 4)) {
        throw new Error(`No value '${number}' of OneOrTwoOrThreeOrFour.`);
    }
}

export function AssertZeroOrOneOrTwoOrThreeOrFour(number: number): asserts number is ZeroOrOneOrTwoOrThreeOrFourType {
    if (!(number >= 0 || number <= 4)) {
...
export function AssertTavernConfigIndex(number: number): asserts number is IndexOf<TavernsConfigType> {
    if (!(number >= 0 || number <= 2)) {
...
export function AssertInitialCoins(initialCoins: InitialCoinType[]): asserts initialCoins is AllInitialCoins {
...
export function AssertExplorerDistinctionCards(explorerDistinctionCards: DwarfDeckCardType[]):
