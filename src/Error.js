...
export const ThrowMyError = (G, ctx, error, ...errorArgs) => {
    ...
    switch (error) {
        ...
        case ErrorNames.CurrentPrivatePlayerIsUndefined:
            throw new Error(`В массиве приватных игроков отсутствует ${G.solo && errorArgs[0] === `1` ? `текущий соло бот` : `текущий игрок`} с id '${errorArgs[0]}'.`);
        case ErrorNames.CurrentPublicPlayerIsUndefined:
            throw new Error(`В массиве публичных игроков отсутствует ${G.solo && errorArgs[0] === `1` ? `текущий соло бот` : `текущий игрок`} с id '${errorArgs[0]}'.`);
        ...
        case ErrorNames.FirstStackActionIsUndefined:
            throw new Error(`В массиве стека действий текущего ${G.solo && ctx.currentPlayer === `1` ? `соло бота` : `игрока`} с id '${ctx.currentPlayer}' отсутствует '0' действие.`);
        ...
        case ErrorNames.PrivatePlayerWithCurrentIdIsUndefined:
            throw new Error(`В массиве приватных игроков отсутствует ${G.solo && errorArgs[0] === `1` ? `соло бот` : `игрок`} с id '${errorArgs[0]}'.`);
        case ErrorNames.PublicPlayerWithCurrentIdIsUndefined:
            throw new Error(`В массиве публичных игроков отсутствует ${G.solo && errorArgs[0] === `1` ? `соло бот` : `игрок`} с id '${errorArgs[0]}'.`);
