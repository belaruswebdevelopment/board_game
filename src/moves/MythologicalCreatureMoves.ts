...
import type { CanBeUndefType, CanBeVoidType, InvalidMoveType, IPublicPlayer, Move, MyFnContext, MythologicalCreatureDeckCardType } from "../typescript/interfaces";
...
export const ChooseSuitOlrunMove: Move = ({ G, ctx, playerID, ...rest }: MyFnContext, suit: SuitNames):
    ...
    const isValidMove: boolean =
        IsValidMove({ G, ctx, playerID, ...rest }, StageNames.chooseSuitOlrun, suit);
    if (!isValidMove) {
        return INVALID_MOVE;
    }
...
export const GetMythologyCardMove: Move = ({ G, ctx, playerID, ...rest }: MyFnContext, cardId: number):
    CanBeVoidType<InvalidMoveType> => {
    const isValidMove: boolean =
        IsValidMove({ G, ctx, playerID, ...rest }, StageNames.getMythologyCard, cardId);
    if (!isValidMove) {
        return INVALID_MOVE;
    }
    ...
    const isAdded: boolean = PickCardOrActionCardActions({ G, ctx, playerID, ...rest }, mythologyCard);
