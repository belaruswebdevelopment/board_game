 ...
 */
const CheckSoloBotAndvariCanPickHero = (G, ctx, player) => {
    const playerCards = Object.values(player.cards), heroesLength = player.heroes.length -
 ...
 */
export const CheckSoloBotAndvariMustTakeCardToPickHero = (G, ctx, moveArguments) => {
    ...
    const suit = CheckSoloBotAndvariCanPickHero(G, ctx, soloBotPublicPlayer), availableMoveArguments = [];
    if (suit !== undefined) {
 ...
 */
const CheckSoloBotAndvariMustTakeCardFromCurrentStrategy = (G, ctx, moveArguments, suit) => {
 ...
 */
export const CheckSoloBotAndvariMustTakeCardFromGeneralStrategy = (G, ctx, moveArguments) => {
    ...
    for (let i = 0; i < G.soloGameAndvariStrategyVariantLevel; i++) {
        ...
        const strategyArguments = CheckSoloBotAndvariMustTakeCardFromCurrentStrategy(G, ctx, moveArguments, suit);
 ...
 */
export const SoloBotMustTakeCardFromReserveStrategy = (G, ctx, moveArguments) => {
    if (G.soloGameAndvariStrategyVariantLevel === null) {
        throw new Error(`Не задан вариант уровня сложности для стратегий соло бота Андвари в соло игре.`);
    }
 ...
 */
export const CheckSoloBotAndvariMustTakeRoyalOfferingCard = (G, ctx, moveArguments) => {
