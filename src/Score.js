 ...
 */
export const CurrentScoring = ({ G, ctx, playerID, ...rest }) => {
    ...
    for (suit in suitsConfig) {
        ...
        if (G.expansions.Idavoll.active) {
            additionalScoring = CheckPlayerHasBuff({ G, ctx, playerID, ...rest }, MythicalAnimalBuffNames.RatatoskFinalScoring);
        }
 ...
 */
const FinalScoring = ({ G, ctx, playerID, ...rest }, warriorDistinctions) => {
    const player = G.publicPlayers[Number(playerID)];
    if (player === undefined) {
        return ThrowMyError({ G, ctx, ...rest }, ErrorNames.PublicPlayerWithCurrentIdIsUndefined, playerID);
    }
    ...
    let score = CurrentScoring({ G, ctx, playerID, ...rest }), coinsValue = 0;
    ...
    if (warriorDistinctions.length && warriorDistinctions.includes(Number(playerID))) {
        const warriorDistinctionScore = StartDistinctionAwarding({ G, ctx, playerID: String(playerID), ...rest }, suitsConfig[SuitNames.warrior].distinction.awarding);
    ...
    const minerDistinctionPriorityScore = StartDistinctionAwarding({ G, ctx, playerID: String(playerID), ...rest }, suitsConfig[SuitNames.miner].distinction.awarding);
    ...
    for (let i = 0; i < player.heroes.length; i++) {
        ...
        if (((G.mode === GameModeNames.Basic || G.mode === GameModeNames.Multiplayer
            ...
            const currentHeroScore = StartHeroScoring({ G, ctx, playerID, ...rest }, heroData.scoringRule);
        ...
        for (let i = 0; i < player.campCards.length; i++) {
            ...
            switch (campCard.type) {
                case RusCardTypeNames.Artefact_Card:
                    currentArtefactScore = StartArtefactScoring({ G, ctx, playerID, ...rest }, artefactsConfig[campCard.name].scoringRule);
    ...
    if (G.expansions.Idavoll.active) {
        ...
        for (let m = 0; m < cards.length; m++) {
            ...
            const mythicalAnimalCard = mythicalAnimalConfig[playerMythicalAnimalCard.name], currentMythicalAnimalScore = StartMythicalAnimalScoring({ G, ctx, playerID, ...rest }, mythicalAnimalCard.scoringRule);
...
export const ScoreWinner = ({ G, ctx, ...rest }) => {
    ...
    for (let i = 0; i < ctx.numPlayers; i++) {
        G.totalScore.push(FinalScoring({ G, ctx, playerID: String(i), ...rest }, warriorDistinctions));
    }
